...
uses libc, BaseUNIX, Strings, FUSE;

const
  hello_path : String = '/hello';
  hello_str : String = 'Hello World!'#10;

function hello_getattr(aNameC : PChar; aSTAT : TStatP) : Integer; cdecl;
var
  aName : String;
begin
  Result := 0;
  ...
  if (aName = '/') then begin
    ...
    aSTAT^.st_mode := S_IFREG or 292; // 0444;
    aSTAT^.st_nlink := 1;
...
function hello_readdir(aNameC : PChar; aBuffer : Pointer; filler : TDirectoryFiller; aFileOffset : off_t; aFileInfo : TFileInfoP) : Integer; cdecl;
begin
...
function hello_open(aNameC : PChar; aFileInfo : TFileInfoP) : Integer; cdecl;
var
  aName : String;
begin
  aName := aNameC;
...
function hello_read(aNameC : PChar; aBuffer : Pointer; aBufferSize : size_t; aFileOffset : off_t; aFileInfo : TFileInfoP) : Integer; cdecl;
var
  len : size_t;
  aName : String;
begin
  ...
  if aName <> hello_path then
    ...
    if (aFileOffset < len) then begin
      ...
      memcpy(aBuffer, PChar(hello_str) + aFileOffset, aBufferSize);
    end else
      aBufferSize := 0;
